...
private var xMvt : float;
private var yMvt : float;
...
function Update () {
	...
	if (healthScript != null){
		...
		if(timeAfterHit < maxRecoil && lastHitTime != 0){
		    ...
		    recoilAmountTarget *= (1- Mathf.Abs(yMvt));
		    recoilAmountTarget *= (1- Mathf.Abs(xMvt));
		}
	...
	if (isGrounded){
		targetForwardSpeed = yMvt;
		targetStrafeSpeed = xMvt;
		targetForwardSpeed *=  forwardSpeedMultiplier;
		targetStrafeSpeed *= strafeSpeedMultiplier;
		if(yMvt < 0){//Slow down going backwards;
