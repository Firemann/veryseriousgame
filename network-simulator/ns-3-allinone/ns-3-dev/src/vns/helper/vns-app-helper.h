#ifndef VNS_APP_HELPER_H
#define VNS_APP_HELPER_H
 ...
 */
class VnsAppClientHelper
{
public:
  /**
   * Create VnsAppClientHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   * \param port The port the server will wait on for incoming packets
   */
  VnsAppClientHelper (uint16_t port, int64_t activated);
  ...
  /**
   * Create a VnsAppClientApplication on the specified Node.
   *
  ...
  /**
   * Create a VnsAppClientApplication on specified node
   *
 ...
 */
class VnsAppAntennaHelper
{
public:
  /**
   * Create VnsAppAntennaHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   * \param ip The IP address of the remote udp echo server
   * \param port The port number of the remote udp echo server
   */
  VnsAppAntennaHelper (Address ip, uint16_t port);
  VnsAppAntennaHelper (Ipv4Address ip, uint16_t port);
  VnsAppAntennaHelper (Ipv6Address ip, uint16_t port);
  ...
  /**
   * Given a pointer to a VnsAppAntenna application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be VnsAppAntenna).
  ...
  /**
   * Create a vns antenna application on the specified node.  The Node
   * is provided as a Ptr<Node>.
   *
   * \param node The Ptr<Node> on which to create the VnsAppAntennaApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the VnsAppAntennaApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
